# set working directory to save your files
...
# files is a list of all 19 .cov file locations
files19 <- c(
...
# gene annotation: annotate cytosine to nearest gene TSS
# this is so we can plot methylation by distance to TSS
...
### FILTER COVERAGE: set threshold of 5x in all samples (you can change this)
...
############ MDS PLOT #############
# shows how the different samples cluster -- we want to know whether CRS ones cluster together or not
# M-value
...
# this pdf() function saves the image as a PDF file to the working directory you set
...
# write.csv() saves a .csv file (that you can open in excel) to your computer
...
# show number of differentially methylated CpG sites at Bonferroni corrected p < 0.05
# Bonferroni is a type of multiple hypothesis testing correction
...
# visualize MD plots -- these are FIGURE 8 IN THESIS
...
#####################################
# manhattan plot -- FIGURE 9 IN THESIS
...
# CRS
chr <- lrt.CRS19$genes$Chr # get chromosome names
chr <- str_remove(chr, "chr") # chr1 --> 1
bp <- lrt.CRS19$genes$Locus # get locus
p <- lrt.CRS19$table$PValue # get p value
gene <- lrt.CRS19$genes$Symbol # get gene symbol
...
### CRS - read in M values from what we previously saved as ManhattanCRS19.txt
xCRS <- read.table("/Volumes/jaeyoonc/_THESIS/JCthesis/ManhattanCRS19.txt", header = TRUE)
colnames(xCRS) <- c("CHR", "BP", "P", "SNP")

# generate manhattan plots -- FIGURE 9 IN THESIS
...
# generate manhattan plot -- FIGURE 9 IN THESIS
...
# annotate by gene parts -- THIS IS FIGURE 11 IN THESIS
...
# visualize pie chart -- THIS IS FIGURE 11 IN THESIS
...
# visualize -- THIS IS FIGURE 10 IN THESIS
...
# visualize output -- THIS IS FIGURE 10 IN THESIS
